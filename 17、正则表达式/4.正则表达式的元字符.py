...
[^sunck]      匹配除了sunck这几个字母以外的所有字符，中括号里的^被称为脱字符，表示
不匹配集合中的字符
[^0-9]        匹配所有的非数字字符
\d            匹配数字，效果同[0-9]
\D            匹配分数字字符，效果同[^0-9]
...
print(re.findall(".","sunck is \na good man 9 ", flags=re.S))

print("-----------------------锚字符（边界字符）----------------------------")
'''
^   行首匹配，和在[]里的^不是一个意思
$   行尾匹配，
\A  匹配字符串开始，它和^的区别是，\A只匹配整个字符串的开头，即使在re.M模式下也不会匹配
其它行的行首
\Z  匹配字符串开始，它和$的区别是，\Z只匹配整个字符串的结束，即使在re.M模式下也不会匹配
其它行的行尾
...
print("---------------------匹配多个字符-------------------------------")
'''
说明：下面的x、y、z均为假设的普通字符，n、m为非负整数，不是正则表达式的元字符
(xyz）   匹配小括号内的xyz（作为一个整体去匹配）
x?      匹配0个或者1个x
x*      匹配0个或者任意多个x  （.* 表示匹配零个或者任意多个字符，换行符除外）
x+      匹配至少一个x
...
print(re.findall(r"a{3,6}","aabaaaaaa"))
...
print("----------------------特殊---------------------------")
'''
*?  +?  x?   最小匹配，通常都是尽可能多的匹配，可以使用这种解决贪婪的算法
(?:x)  类似（xyz），但不表示一个组
